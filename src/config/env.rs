...
mod tests {
    ...
    fn test_config_validation() {
        // Test invalid day
        let config = Config {
            gmail_client_id: "test".to_string(),
            gmail_client_secret: "test".to_string(),
            drive_client_id: "test".to_string(),
            drive_client_secret: "test".to_string(),
            drive_folder_path: "test".to_string(),
            fetch_invoices_day: 32,
        };
        ...
        let config = Config {
            ...
            target_keywords: vec!["invoice".to_string(), "fatura".to_string()],
        };
    ...
    fn test_config_from_env() {
        ...
        env::set_var("GOOGLE_GMAIL_CLIENT_ID", "test-gmail-id");
        env::set_var("GOOGLE_GMAIL_CLIENT_SECRET", "test-gmail-secret");
        env::set_var("GOOGLE_DRIVE_CLIENT_ID", "test-drive-id");
        env::set_var("GOOGLE_DRIVE_CLIENT_SECRET", "test-drive-secret");
        env::set_var("GOOGLE_DRIVE_FOLDER_LOCATION", "billing/2025");
        env::set_var("FETCH_INVOICES_DAY", "5");
        env::set_var("TARGET_KEYWORDS_TO_FETCH_AND_DOWNLOAD", "invoice, fatura");

        let config = Config::from_env().unwrap();
        assert_eq!(config.gmail_client_id, "test-gmail-id");
        assert_eq!(config.fetch_invoices_day, 5);
        assert_eq!(config.target_keywords, vec!["invoice", "fatura"]);
    }
}
